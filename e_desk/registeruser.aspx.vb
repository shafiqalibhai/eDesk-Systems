...
#End Region
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Dim con As New SqlConnection("server=(local);uid=sa;database=eDesk")
        'con.Open()
        ...
        Else
            con.Close()
            'con.Open()
            str = "insert into registration values('" & s1 & "','" & s2 & "','" & s3 & "','" & s4 & "','" & s5 & "','" & s6 & "','" & s7 & "'," & s8 & ",'" & s9 & "','" & s10 & "','" & s11 & "','" & s12 & "','" & s13 & "','user')"
            'str = "insert into registration (name,address,phoneno) values('" & s1 & "','" & s2 & "','" & s3 & "')"
